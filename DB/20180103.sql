...
insert  into `asistencias`(`idasistencia`,`idpersona`,`ingreso`,`salida`,`accion`,`fechaaccion`,`idempleadoaccion`,`idpunto`,`idpuntoaccion`) values 
(2,70,'2017-12-29 04:08:26',NULL,'I','2017-12-29 04:08:42',1,2,1),
(3,70,'2017-12-29 04:25:08',NULL,'I','2017-12-29 04:25:21',1,2,1),
(4,70,'2017-12-29 04:25:54',NULL,'I','2017-12-29 04:26:02',1,2,1),
(5,70,'2017-12-29 04:26:12',NULL,'I','2017-12-29 04:26:12',1,2,1),
(6,70,'2018-01-02 08:54:22',NULL,'I','2018-01-02 08:54:22',1,2,1);
...
(45,'2018-01-02 00:00:00','0','100','Pago de cuota',83,'N','2018-01-02 09:13:33',1,1,0);
...
(26,24,'Yoga funcional','M','2018-01-03 04:37:33',1,1),
...
(78,29,'2018-01-03',NULL,'800',NULL,'400','M','2017-12-21 01:37:10',1,4,1),
(83,31,'2018-01-02',NULL,'800',NULL,'400','M','2018-01-02 09:13:33',1,1,1);
...
(31,24,2,70,'N','2018-01-02 09:11:37',1,1);
...
(24,'Grupal cambio','M','2018-01-03 04:37:33',1,1);
...
when 'I' then
	...
	VALUE (_idpersona,_ingreso,_idpunto,_accion,_fechaaccion,_idempleadoacc,_idpuntoacc);
WHEN 'S' THEN
...
select idasistencia from asistencias where idpersona=_idpersona and idpunto=_idpunto and fechaaccion=_fechaaccion and accion=_accion;
END */$$
DELIMITER ;
...
ELSE
	...
	ELSE
		...
		PREPARE STMT FROM "SELECT a.idcajadeldia,a.fechacarga AS 'Fecha Carga',d.nombrepunto AS 'Punto',a.detallemovimiento AS 'Detalle Movimiento',c.nombre AS 'Proveedor','' AS 'Clase/Tipo',a.montoingreso AS 'Ingresos',a.montoegreso AS 'Egresos',a.idcuota,a.idgasto
			...
			DEALLOCATE PREPARE STMT;
		ELSE
			IF (_tipooperacion = 'Asistencias') THEN
			PREPARE STMT FROM "SELECT a.idasistencia,a.ingreso AS 'Fecha/Hora Ingreso',a.salida AS 'Fecha/Hora Salida',c.nombrepunto AS 'Punto',CONCAT(b.apellido,', ',b.nombre) AS 'Persona',d.tipopersona AS 'Tipo Persona',
						 '' AS 'col1',
						(SELECT SUM(f.montosaldo) 
						 FROM cursos e INNER JOIN cuotas f ON (f.idcurso=e.idcurso AND e.accion!='B' AND f.accion!='B')
						 WHERE (e.idpersona=a.idpersona) AND (f.montopagado IS NULL) AND (f.fechapago<=?)) AS 'Deuda',
						 '' AS 'col2',
						 '' AS 'col3',
							 b.fechafincert AS 'Aptitud Medica'
					   FROM asistencias a INNER JOIN personas b ON (a.idpersona=b.idpersona AND b.accion!='B')
					                      INNER JOIN puntos c ON (c.idpunto=a.idpunto AND c.accion!='B')
					                      INNER JOIN tipopersonas d ON (d.idtipopersona=b.idtipopersona)
					   WHERE (a.accion!='B') AND 
						 (a.ingreso BETWEEN ? AND ?) AND 
						 (d.tipopersona LIKE CONCAT('%',? ,'%')) AND
						 (b.apellido LIKE CONCAT('%',? ,'%')) AND
						 (b.nombre LIKE CONCAT('%',? ,'%')) AND
						 (c.nombrepunto LIKE CONCAT('%',? ,'%')) 
					   ORDER BY 2
					   LIMIT ?,?";
				
				SET @fechapago = _fpago; 
				SET @fechaini = _fechaini; 
				SET @fechafin = _fechafin; 
				SET @tipopersona = _tipopersona; 
				SET @apellido = _apellido; 
				...
				SET @pag = _pag; 
				SET @cantfil = _cantfil; 
				EXECUTE STMT USING @fechapago,@fechaini,@fechafin,@tipopersona,@apellido,@nombre,@punto,@pag,@cantfil;
				DEALLOCATE PREPARE STMT;
			ELSE
...
BEGIN
	SELECT a.idpersona,CONCAT(a.apellido,', ', a.nombre) AS Nombre,b.tipopersona AS Privilegio,a.fechafincert as aptomedico,a.urlfoto,c.nombrepunto
	FROM personas a INNER JOIN tipopersonas b ON (a.idtipopersona=b.idtipopersona)
	INNER JOIN puntos c on (a.idpunto=c.idpunto)
