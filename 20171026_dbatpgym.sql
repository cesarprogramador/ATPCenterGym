...
ELSE
	...
	ELSE
		...
		PREPARE STMT FROM "SELECT a.idcajadeldia,a.fechacarga AS 'Fecha Carga',d.nombrepunto AS 'Punto',a.detallemovimiento AS 'Detalle Movimiento',c.nombre AS 'Persona','' AS 'Clase/Tipo',a.montoingreso AS 'Ingresos',a.montoegreso AS 'Egresos',a.idcuota,a.idgasto
				   FROM cajadeldia a INNER JOIN gastos b ON (a.idgasto=b.idgasto AND b.accion!='B')
						     INNER JOIN personas c ON (c.idpersona=b.idpersona AND c.accion!='B')
						     INNER JOIN puntos d ON (d.idpunto=b.idpunto AND d.accion!='B')
				   WHERE (a.accion!='B') AND 
			...
			DEALLOCATE PREPARE STMT;
	ELSE
		PREPARE STMT FROM "SELECT '' as col0,a.fechapago AS 'Fecha Pago',d.nombrepunto AS 'Punto',a.montocuota AS 'Cuota',a.montosaldo AS 'Saldo',a.montocuota+a.montosaldo AS 'Deuda', CONCAT('Debe Cuota Nº',a.cuotanum) AS 'Detalle Movimiento',CONCAT(e.apellido,', ',e.nombre) AS 'Persona',CONCAT(c.nombreclase,'/',f.tipodeclase) AS 'Clase/Tipo',a.idcuota
